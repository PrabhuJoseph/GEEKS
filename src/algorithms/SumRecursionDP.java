...
class SumRecursionDP
{
                       ...
                       int required_sum, int n)
    {
        ...
        if (v[i][required_sum + base]) {
            System.out.println("RETURN="+(required_sum + base)+"---"+dp[i][required_sum + base]);
        ...
        System.out.println("FIND FOR="+(required_sum + base));
        // Recurrence relation
        dp[i][required_sum + base] =
                findCnt(arr, i + 1, required_sum, n) +
                        findCnt(arr, i + 1, required_sum - arr[i], n);
        System.out.println("RETURN FOR="+(required_sum + base)+"---"+dp[i][required_sum + base]);
        return dp[i][required_sum + base];
    }
